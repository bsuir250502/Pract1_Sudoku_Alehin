...
int check_value(int cell_num, int value, int *array, int *outputArray)
{
    int i;
    for (i = 0; i < PEER_LEN; i++) {
        if (value == outputArray[peers[cell_num][i]])
            return 0;
    }
    return value;
}

int backtrack(int cell_num, int *array, int *outputArray)
{
    int temp, i;
    if (outputArray[cell_num] == 0) {
        for (i = 1; i < 10; i++) {
            temp = check_value(cell_num, i, array, outputArray);
            if (temp > 0) {
                outputArray[cell_num] = temp;
                if (cell_num == 80) {
                    return 1;
                } else {
                    if (backtrack(cell_num + 1, array, outputArray)) {
                        return 1;
                    }
                }
            }
        ...
        if (i == 10) {
            if (outputArray[cell_num] != array[cell_num]) {
                outputArray[cell_num] = 0;
            }
            return 0;
        }
    } else {
